from utils import get_employs, get_employ_vacancies, create_db, create_table_employ, insert_table_data
...
def main():
    ...
    names_employers = ["Villacarte", "idaproject", "Secreate",
                       "getmatch", "Фрифлекс", "Staffwell", "Remokate",
                       "ECCO", "СберМаркет", "MyCleaning"]
    employers = get_employs(names_employers)
    create_table_employ(params, employers)
    print("Таблицы успешно созданы")
    index_employer = 1
    for employer in employers:
        employer_vacancies = get_employ_vacancies(employer["id"])["items"]
